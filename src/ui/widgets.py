...
class ClickableTextWidget(tk.Frame):
    ...
    def __init__(
        ...
        self.folder_patterns = {
            ...
            "trailing_slash_relative": re.compile(
                r'(?:\.{0,2}[\\\/])+(?:[^\\\/\n\r<>"|*?]+[\\\/])+'
            ),
            "trailing_slash_network": re.compile(
                r'\\\\[^\\\/\n\r<>"|*?]+\\(?:[^\\\/\n\r<>"|*?]+[\\\/])+'
            ),
            # 확장자 없는 경로 (추정 폴더)
            "no_extension_absolute": re.compile(
                r'[A-Za-z]:[\\\/](?:[^\\\/\n\r<>"|*?]+[\\\/])*[^\\\/\n\r<>"|*?]+(?!\.[A-Za-z0-9])'
            ),
            "no_extension_relative": re.compile(
                r'(?:\.{1,2}[\\\/])+(?:[^\\\/\n\r<>"|*?]+[\\\/])*[^\\\/\n\r<>"|*?]+(?!\.[A-Za-z0-9])'
            ),
            "no_extension_network": re.compile(
                r'\\\\[^\\\/\n\r<>"|*?]+\\(?:[^\\\/\n\r<>"|*?]+[\\\/])*[^\\\/\n\r<>"|*?]+(?!\.[A-Za-z0-9])'
            ),
        }
    ...
    def _get_link_type_and_matches(self, text):
        ...
        for match in self.url_pattern.finditer(text):
            matches.append({"match": match, "type": "url", "content": match.group()})
        ...
        for pattern_name, pattern in self.file_patterns.items():
            for match in pattern.finditer(text):
                ...
                for existing_match in matches:
                    if (
                        match.start() < existing_match["match"].end()
                        and match.end() > existing_match["match"].start()
                    ):
                        is_overlap = True
                        break
    ...
    def _setup_clickable_text(self):
        ...
        if self._debug:
            try:
                ...
                print(
                    f"[DEBUG] Found {len(all_matches)} links in text (encoding issue prevented full display)"
                )
        ...
        for i, link_info in enumerate(all_matches):
            ...
            self.text_widget.tag_configure(
                tag_name, foreground=color, underline=True, font=self.font_info + ("underline",)
            )
            ...
            self.text_widget.tag_bind(
                tag_name, "<Enter>", lambda e: self.text_widget.configure(cursor="hand2")
            )

            self.text_widget.tag_bind(
                tag_name, "<Leave>", lambda e: self.text_widget.configure(cursor="arrow")
            )
        ...
        try:
            ...
            if file_extension in dangerous_extensions:
                if self._debug:
                    try:
                        print(
                            f"[DEBUG] 위험한 실행 파일 감지: {file_path} (확장자: {file_extension})"
                        )
                    except UnicodeEncodeError:
                        print(
                            f"[DEBUG] Dangerous executable detected: {file_path} (ext: {file_extension})"
                        )
...
class TodoItemWidget(tk.Frame, DragDropMixin):
    ...
    def _update_due_date_display(self):
        ...
        # 라벨 업데이트
        self.due_date_label.configure(text=display_text, fg=fg_color, bg=bg_color)
    ...
    def _update_completion_style(self):
        ...
        if self.todo_data["completed"]:
            # 완료된 항목: 취소선과 흐린 텍스트
            self.text_widget.text_widget.configure(fg=colors["text_secondary"])
            ...
            # 미완료 항목: 일반 스타일
            self.text_widget.text_widget.configure(fg=colors["text"])
    ...
    def _edit_todo_dialog(self, event=None):
        ...
        try:
            ...
            dialog = DatePickerDialog(
                self.winfo_toplevel(), current_text, current_date, edit_mode=True
            )
        ...
        if hasattr(self, "text_widget") and hasattr(self.text_widget, "text_widget"):
            if completed:
                ...
                self.text_widget.text_widget.configure(
                    fg=self.colors["text_secondary"], font=("Segoe UI", 9, "overstrike")
                )
            else:
                # 미완료 항목: 일반 스타일
                self.text_widget.text_widget.configure(fg=self.colors["text"], font=("Segoe UI", 9))
...
def get_button_style(button_type: str = "primary") -> Dict[str, Any]:
    """버튼 스타일 반환"""
    base_style = {"font": ("Segoe UI", 9), "border": 0, "relief": "flat", "cursor": "hand2"}
    ...
    if button_type == "primary":
        ...
        base_style.update(
            {"bg": DARK_COLORS["danger"], "fg": "white", "activebackground": "#dc2626"}
        )
