# -*- coding: utf-8 -*-
"""
Windows TODO Panel 빌드 스크립트

PyInstaller를 사용해서 Windows TODO Panel 애플리케이션을 
단일 exe 파일로 빌드합니다.
...
사용법:
python build.py [--debug] [--no-upx] [--keep-temp]

옵션:
--debug     : 디버그 모드로 빌드 (콘솔 표시)
--no-upx    : UPX 압축 사용하지 않음
...
class BuildManager:
    ...
    def __init__(self, debug: bool = False, use_upx: bool = True, keep_temp: bool = False):
        self.debug = debug
        ...
        self.spec_file = self.root_dir / 'todo_panel.spec'
        self.icon_file = self.root_dir / 'TodoPanel.ico'
        self.version_file = self.root_dir / 'version_info.txt'
        self.output_exe = self.dist_dir / 'TodoPanel.exe'
    ...
    def check_requirements(self) -> bool:
        ...
        if self.use_upx:
            if shutil.which('upx'):
                self.log("UPX 압축기 확인됨")
            else:
                self.log("UPX 압축기를 찾을 수 없습니다. 압축 없이 빌드합니다.", "WARNING")
                self.use_upx = False
        ...
        try:
            ...
            for pattern in temp_files:
                for file_path in Path(pattern.parent).glob(pattern.name):
                    try:
                        file_path.unlink()
                        self.log(f"임시 파일 삭제됨: {file_path}")
                    except:
                        pass
    ...
    def create_build_command(self) -> List[str]:
        """PyInstaller 빌드 명령어 생성"""
        cmd = ['pyinstaller']
        ...
        if self.version_file.exists():
            cmd.extend(['--version-file', str(self.version_file)])
        ...
        # 추가 경로 설정
        cmd.extend(['--add-data', f'{self.src_dir};.'])
        cmd.extend(['--paths', str(self.src_dir)])
        
        # 숨겨진 import 설정
        hidden_imports = [
            'pystray', 'PIL', 'PIL.Image', 'PIL.ImageDraw',
            'psutil', 'tkinter', 'tkinter.ttk', 'tkinter.messagebox',
            'uuid', 'json', 'logging', 'threading', 'datetime'
        ]
        ...
        # 제외할 모듈들
        exclude_modules = [
            'test_todo_manager', 'test_main', 'simple_test'
        ]
    ...
    def run_pyinstaller(self) -> bool:
        ...
        try:
            # spec 파일이 있으면 사용, 없으면 명령행 옵션 사용
            if self.spec_file.exists():
                self.log(f"spec 파일 사용: {self.spec_file}")
                cmd = ['pyinstaller', str(self.spec_file)]
                
                # spec 파일을 사용할 때는 추가 옵션 불가
                if self.debug:
                    self.log("디버그 모드는 spec 파일 내에서 설정됩니다", "WARNING")
                if not self.use_upx:
                    self.log("UPX 설정은 spec 파일 내에서 관리됩니다", "WARNING")
            else:
                self.log("명령행 옵션으로 빌드")
    ...
    def print_build_summary(self):
        ...
        if self.output_exe.exists():
            ...
            print(f"- UPX 압축     : {'예' if self.use_upx else '아니오'}")
    ...
    def build(self) -> bool:
        """전체 빌드 프로세스 실행"""
        print("=" * 60)
        print("Windows TODO Panel 빌드 시작")
        print("=" * 60)
        print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Windows TODO Panel 빌드 스크립트',
